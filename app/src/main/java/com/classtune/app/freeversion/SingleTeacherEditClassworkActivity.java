...
public class SingleTeacherEditClassworkActivity extends ChildContainerActivity {
    ...
    public void PublishHomeWork() {
        if(!TextUtils.isEmpty(selectedFilePath)){
            ...
            ApplicationSingleton.getInstance().getNetworkCallInterface().teacherAddClassworkEdit(user_secret, subject_id, content, title, type, ids, body, mime_type, file_size).enqueue(
                    new Callback<JsonElement>() {
                        @Override
                        public void onResponse(Call<JsonElement> call, Response<JsonElement> response) {
                            if (uiHelper.isDialogActive())
            ...
            ApplicationSingleton.getInstance().getNetworkCallInterface().teacherAddClassworkEdit(UserHelper.getUserSecret(), subjectId, classworkDescriptionEditText
                    .getText().toString(), subjectEditText.getText().toString(), homeworkTypeId, id).enqueue(
                    new Callback<JsonElement>() {
                        @Override
                        public void onResponse(Call<JsonElement> call, Response<JsonElement> response) {
                            if (uiHelper.isDialogActive())
